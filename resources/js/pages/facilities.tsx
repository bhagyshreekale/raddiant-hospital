...
export default function FacilitiesPage() {
  ...
  const filteredTpa = tpaFilter === 'all' ? TPA_LIST : TPA_LIST.filter((t) => t.cat === tpaFilter);
            ...
            {HEALTH_PACKAGES.map((pkg) => (
              ...
              >
                ...
                <div className="my-5 border-b pb-5" style={{ borderColor: 'rgba(255,255,255,0.15)' }}>
                  <span className="font-serif text-3xl font-semibold" style={{ color: '#ffffff' }}>{pkg.price}</span>
                </div>
                <ul className="mb-7 space-y-2.5">
                  {pkg.features.map((f) => (
                    <li key={f} className="flex items-center gap-2.5 text-xs" style={{ color: 'rgba(255,255,255,0.75)' }}>
                      <span
                        className={`flex h-4 w-4 flex-shrink-0 items-center justify-center rounded-full text-[9px] font-bold
                          ${pkg.featured ? 'bg-white/25' : 'bg-white/15'}`}
                      >
                        ✓
                      </span>
                      {f}
