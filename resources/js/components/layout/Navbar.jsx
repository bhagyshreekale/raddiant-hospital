...
const DEFAULT_SITE_DATA = {
  ...
  emergency: "108",
  // Updated to match the new dynamic array structure from the backend
  nav: [
    { label: "Home", href: "/" },
...
export default function Navbar() {
  ...
  // 1. Ensure nav is treated as an array (using our new dynamic backend structure)
  const allNavLinks = Array.isArray(data.nav) ? data.nav : DEFAULT_SITE_DATA.nav;

  // 2. Intelligently separate standard links from the CTA (Call To Action) button.
  // If the admin adds an item with "Appointment" or "Book" in the label, we render it as the blue button.
  const standardLinks = allNavLinks.filter(
    link => !link.label.toLowerCase().includes("appointment") && !link.label.toLowerCase().includes("book")
  );
  const ctaLink = allNavLinks.find(
    link => link.label.toLowerCase().includes("appointment") || link.label.toLowerCase().includes("book")
  );
      ...
      >
        <div className="container mx-auto px-4 flex items-center justify-between py-2.5">
          <a href="/" className="no-underline flex items-center gap-2">
            <img src="/images/logo.png" alt={data.name} width={160} height={85} className="rounded-lg object-contain" />
          </a>

          {/* Desktop links */}
          <div className="hidden lg:flex items-center gap-1">
            {standardLinks.map((link, index) => (
              <a key={index} href={link.href} className="px-3.5 py-2 rounded-lg text-gray-700 font-medium text-[0.9rem] transition-all duration-200 hover:bg-gray-100 hover:text-blue-600">
                {link.label}
              </a>
            ))}
            ...
            {ctaLink && (
              <a href={ctaLink.href} className="ml-2 bg-blue-600 text-white px-5 py-2.5 rounded-lg text-[0.88rem] font-semibold hover:bg-blue-700 transition-all shadow-sm active:scale-95">
                {ctaLink.label}
              </a>
            )}
          </div>
        ...
        <div className={`lg:hidden bg-white border-t border-gray-200 overflow-hidden transition-all duration-300 ${menuOpen ? "max-h-[500px] py-4" : "max-h-0"}`}>
          <div className="container mx-auto px-4 flex flex-col gap-1">
            {standardLinks.map((link, index) => (
              <a key={index} href={link.href} onClick={() => setMenuOpen(false)} className="px-4 py-2.5 rounded-lg text-gray-700 font-medium text-[0.95rem] hover:bg-gray-100 transition-colors">
                {link.label}
              </a>
            ))}
            
            {/* Dynamic Call to Action Button (Mobile) */}
            {ctaLink && (
              <a href={ctaLink.href} className="mt-2 bg-blue-600 text-white text-center py-3 rounded-lg font-semibold active:bg-blue-700 transition-colors" onClick={() => setMenuOpen(false)}>
                {ctaLink.label}
              </a>
            )}
          </div>
